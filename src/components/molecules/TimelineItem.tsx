...
export function TimelineItem({ item, index, isLast, onFocus }: TimelineItemProps) {
  ...
  return (
    ...
    >
      ...
      {!isLast && (
        <div 
          className="absolute left-1/2 top-16 w-0.5 h-16 bg-gradient-to-b from-slate-300 to-transparent dark:from-slate-600 transform -translate-x-1/2 md:block hidden"
          style={{
            opacity: isIntersecting ? 1 : 0,
      ...
      <div className={`flex-1 ${isLeft ? 'md:pr-8' : 'md:pl-8'}`}>
        <div 
          ref={cardRef}
          className={`bg-white dark:bg-slate-800 rounded-lg shadow-lg p-6 border-2 transition-all duration-300 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900 ${
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className={`flex items-center ${isLeft ? 'md:flex-row-reverse' : 'flex-row'}`}>
              <div 
                className="p-2 rounded-full text-white mr-3 md:mr-0 flex-shrink-0"
            ...
            <div className={`text-sm text-slate-500 dark:text-slate-400 flex-shrink-0 ${isLeft ? 'md:text-left' : 'md:text-right'}`}>
          ...
          {item.technologies && item.technologies.length > 0 && (
            <div className={`flex flex-wrap gap-2 mb-4 ${isLeft ? 'md:justify-end' : 'justify-start'}`}>
            ...
            >
              {item.details && (
                <div className="mb-4">
                  ...
                  <ul className={`text-sm text-slate-600 dark:text-slate-400 space-y-1 ${isLeft ? 'md:text-right' : 'text-left'}`}>
      ...
      <div className="absolute left-1/2 top-8 transform -translate-x-1/2 z-10 hidden md:block">
        <div 
          className="w-4 h-4 rounded-full border-4 border-white dark:border-slate-900"
          style={{ 
